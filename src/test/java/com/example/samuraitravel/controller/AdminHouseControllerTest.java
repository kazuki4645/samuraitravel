...
public class AdminHouseControllerTest {
	...
	@Test
	@WithUserDetails("taro.samurai@example.com")
	public void 一般ユーザーとしてログイン済みの場合は管理者用の民宿詳細ページが表示されずも403エラーが発生する() throws Exception {
		mockMvc.perform(get("/admin/houses/1"))
			   .andExpect(status().isForbidden());	
	}
	...
	@Test
	@WithUserDetails("hanako.samurai@example.com")
	public void 管理者ユーザーとしてログイン済みの場合は管理者用の民宿登録ページが正しく表示される() throws Exception {
		mockMvc.perform(get("/admin/houses/register"))
			   .andExpect(status().isOk())
	...
	@WithUserDetails("hanako.samurai@example.com")
	@Transactional
	public void 管理者ユーザーとしてログイン済みの場合は民宿登録後に民宿一覧ページにリダイレクトする() throws Exception {
		// テスト前のレコード数を取得する
		long countBefore = houseService.countHouses();
	...
	@WithUserDetails("hanako.samurai@example.com")
	@Transactional
	public void 管理者ユーザーとしてログイン済みの場合は民宿更新後に民宿詳細ページにリダイレクトする() throws Exception {
		// テスト用の画像ファイルデータを準備する
		Path filePath = Paths.get("src/main/resources/static/storage/house01.jpg");
		String fileName = filePath.getFileName().toString();
		String fileType = Files.probeContentType(filePath);
